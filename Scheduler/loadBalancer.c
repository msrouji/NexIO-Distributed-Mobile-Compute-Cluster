    ...
    switch (functionality) {
      ...
      case (RESULT_FILE):
        ...
        if (rv == -1) {
          unregisterDevice(H, sendNodeWorker);
          free(buf);
          printf("RN: Unable to receive result file from device %d, errno: %d\n", H->ID, errno);
          return;
        }
        ...
        printf("RN: Received result file for job: %d and exe: %d from hardware ID %d\n", jobID, exeID, H->ID);
      ...
      case (UNREGISTER):
        printf("RN: Unregistered hardware device %d\n", H->ID);
        unregisterDevice(H, sendNodeWorker);
        pthread_exit(NULL);
        break;
    }
